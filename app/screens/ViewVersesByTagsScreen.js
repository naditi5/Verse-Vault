...
export default function ViewVersesByTagsScreen({ }) {
  ...
  const fetchVerses = async () => {
    const result = await db.getAllAsync(`
      ...
      FROM verses v
      JOIN titles t ON v.id = t.id
      ORDER BY v.id ${sortOrder};
    `);
    setVerses(result);
  ...
  const toggleSort = async () => {
    ...
    const result = await db.getAllAsync(`
      SELECT v.id, v.content, v.page_number, v.date_time, v.tags, t.title, t.author
      FROM verses
...
const styles = StyleSheet.create({
  ...
  author: { fontSize: 14, color: "#6A1B9A", marginTop: 2 },
